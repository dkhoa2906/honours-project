...
\chapter{INTRODUCTION AND CONTEXT}
    ...
    requires dedicated human supervision, and depends on specialised lab facilities - which is
...
\chapter{REVIEW OF EXISTING KNOWLEDGE}
    ...
    physically performing it are not distinct processes. When a person performs 
    Motor Imagery, mentally imagining a movement while remaining physically 
    still, their brain activates many of the same neural networks used during 
    actual execution.
    ...
    hemisphere, while left-hand imagery activates the right.
    ...
    synchronise again, leading to Event-Related Synchronisation (ERS).
    ...
    between individuals. The frequency and spatial distribution of these signals 
    are unique to each person, necessitating a system that can adapt to the user.
    ...
    challenge. Raw scalp EEG is weak, which is in microvolts, and obscured by noise such as 
    muscle movement and electrical interference. Therefore, data must pass 
    through a strict processing pipeline.
    ...
    this is Common Spatial Patterns (CSP). CSP functions like a sophisticated 
    filter that combines signals from different electrodes to create new channels. 
    Its goal is to find a specific blend that maximises the variance of signal 
    power for one task (e.g., right hand) while minimising it for another. This 
    transforms complex biological signals into separable features, simplifying 
    the prediction task for the computer.
    ...
    Vector Machines (SVM) have been the standard classifiers. They remain popular 
    due to their computational efficiency on small datasets, 
    making them ideal for real-time applications.

    Recently, the field has explored Deep Learning techniques, such as 
    Convolutional Neural Networks (CNNs). These models promise to automatically 
    learn features from raw data, potentially bypassing manual steps like CSP. 
    However, Deep Learning models typically require thousands of trials to 
    perform reliably. This is often impractical for real-time BCI usage, where 
    setup time must be minimized. Therefore, traditional pipelines using CSP 
    and LDA remain the most practical choice for achieving high accuracy with 
    ...
    on a screen) for 20 to 30 minutes.

    This phase is compulsory because brain signals are non-stationary. They 
    change constantly due to fatigue, attention levels, and electrode shifts. 
    ...
    induces boredom and cognitive fatigue. As the user tires, their ability to 
    generate distinct neural patterns degrades, leading to poor quality data. 
    This creates a cycle where poor data leads to a poor classifier, frustrating 
    the user and further degrading signal quality.
    ...
    motivation. In BCI, this is typically utilized during Neurofeedback training. 
    ...
    focuses, producing more pronounced ERD/ERS patterns.
    ...
    game designing process is critical. If a game is visually chaotic, it may 
    generate interfering 
    signals. The design must balance engagement with signal purity.
    ...
    Mathematical solutions like Transfer Learning address the lack of data other 
    than
    struggle with biological non-stationarity. Conversely, gamification improves 
    signal quality but is rarely applied to the calibration phase itself.
...
\chapter{PRODUCT DESIGN}
    ...
    signals, this smart module runs in the background to analyze the labeled 
    ...
    \begin{enumerate}
        ...
        \item \textbf{Session B (Standard):} The user performs 15 minutes of standard training (watching arrows).
    ...
    light. This minimizes the risk of technical failure during the implementation 
    phase, ensuring the requirements suitable with an individual project.
...
\chapter{FEASIBILITY, RISKS, AND ETHICAL CONSIDERATIONS}
    ...
    \subsection{Required Resources}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                \item \textbf{Testing Environment:} A quiet, isolated room to minimize external signal artifacts and distraction during data collection.
    ...
    \section{Risks and Mitigation}
        ...
        \begin{table}[h!]
            ...
            \vspace{0.2cm}
            \renewcommand{\arraystretch}{1.3} % Uncomment this line to increase row height if needed
            ...
            \addlinespace % Adds a little gap between rows for readability
            
            \textbf{Technical} & 
            Dry sensors have high noise/impedance. & 
            High & 
            Implement strict artifact removal (8-30Hz) and stillness mechanics. \\ 
            \addlinespace
            ...
            \textbf{Subject} & 
            BCI Illiteracy ($\sim$20\% of users). & 
            Medium & 
